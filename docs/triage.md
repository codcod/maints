...
This command requires the **Global Jira Settings** and **Agent Settings** (`CURSOR_API_KEY`). See the [main README](../README.md#configuration) for details.

### Configuration directory

Files used for triage (checklist, prompt, field mappings, optional knowledge base index) are resolved in this order:
...
By default, up to **5** issues are triaged in parallel. Each triage involves a Jira HTTP round-trip and a `cursor-agent` invocation, so concurrency cuts wall time proportionally. Use `--concurrency` / `-j` to tune the limit:
...
You can extract additional fields from Jira issues by creating a `fields-mapping.json` file in your maints configuration directory. The file should contain a JSON array of mappings, where each mapping has a `field` (display name) and a `path` (dot-notation path to the value in the Jira JSON response).
...
3. **Analyze**: Invokes `cursor-agent` in headless mode with `triaged-maints/KEY/` as its workspace. The agent is instructed via the prompt template to read both files and evaluate the issue against each checklist item.
4. **Report**: The agent's response is printed to stdout and also saved as `triaged-maints/KEY/report-KEY.md`. A machine-readable JSON summary is saved to `triaged-maints/KEY/report-KEY.json`.
