...
Start a server that periodically polls Jira and triages new issues
automatically.

## Configuration

`maints serve` requires the **Global Jira Settings** and **Agent Settings**
(`CURSOR_API_KEY`). See the [main README](../README.md#configuration) for
details.

The file layout under `~/.config/maints/` (checklist, prompt, etc.) applies
exactly the same as for `maints triage`; see the [Triage
documentation](triage.md#configuration-directory) for details.
...
- `-c, --checklist`: Path to the checklist Markdown file (default:
  `./checklist.md`).
...
- `-p, --prompt`: Path to the prompt template Markdown file (default:
  `./triage-prompt.md`).
- `--statuses`: Comma-separated Jira statuses to watch when `--jql` is not set
  (default `Open,Triage`).
