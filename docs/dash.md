...
- `--columns`: Comma‑separated column names (case‑insensitive; optional spaces). Allowed names: `key`, `priority`, `status`, `due`, `summary`, `scheduled` (also `scheduled_version` or `scheduled-version`); `assignee`. The **summary** column defaults to 50 runes; use `summary[N]` to cap at `N` runes (e.g. `summary[20]`). Only `summary` may use a `[N]` suffix. Example: `--columns "key, summary[20], priority"`.
